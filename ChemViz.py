...
class ChemViz:
    ...
    def __specie_encoding_latex(self,specie):
        return re.sub(r"(\d)",r"$_\1$",specie).strip()

    def plot_time_series(self,figsize=(12,5),species=None,max_time=None):
        '''
        returns a series
        '''
        df = self.chemsol.to_df()
        time = df.t.values
        if max_time == None:
            ulim = len(time)
        else:
            ulim = np.sum(time<=max_time)
        time = time[:ulim]
        species_set = set([])
        for col in list(df.columns):
            pos = col.find('-Concentration')
            if pos>0:
                species_set.add(col[:pos])
        if species!=None:
        ...
        for specie in species_set:
            plt.plot(time,df[specie+"-Concentration"].values[:ulim],label=self.__specie_encoding_latex(specie))
